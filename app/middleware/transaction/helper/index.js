...
class TransactionHelper {
  ...
  static createTransactionList(arr) {
    return arr.map((el) => {
      ...
      const cur_percent = TransactionHelper.calcCurPercent(transaction_amount);
      ...
      cur_currency = TransactionHelper.setTransactionsCurrency(cur_currency);
  ...
  static calcNetworkFee(el) {
    return (
      el.fee / 100 || el.fees / 100 || el.transaction_info.fee_amount.value * -1
    );
  }
   ...
   * @memberof TransactionHelper
   */
  static findCurrencyNet(transaction_list, net_NGN, net_USD, net_GBP) {
  ...
  static async fetchApiData(paystack, stripe) {
    try {
      const data = await Promise.all([paystack, stripe]).then((values) => {
