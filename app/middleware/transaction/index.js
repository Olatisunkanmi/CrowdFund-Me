...
const { successResponse, errorResponse, Performance, curTime } = Helper;

const { SUCCESS, THIRD_PARTY_ERROR, INTERNAL_SERVER_ERROR } = constants;
const {
  fetchStripeBalance,
...
const { fetchPaystackBalance, fetchAllPaystackTransactions, chargeCard } =
  PaystackService;
...
class TransactionMiddleware {
  ...
  static async fetchTransactionBalance(req, res, next) {
    try {
      ...
      const total_transactions_paystack = PaystackData.total_transactions;

      const total_volume_NGN = PaystackData.total_volume_by_currency[0].amount;

      const total_volume_USD =
        PaystackData.total_volume_by_currency[1].amount + StripeData[0].amount;
  ...
  static async fetchGiftAidDonations(req, res, next) {
    ...
    try {
      ...
      let PaystackData = (await fetchAllPaystackTransactions()).data.data;
      let PaypalData = (await fetchAllPaypalTransactions(start, end)).data
        .transaction_details;
      ...
      Data = Data.map((el) => {
        ...
        let cur_status =
          el.paid || el.status || el.transaction_info.transaction_status;
        ...
        const transaction_amount =
          el.amount || el.transaction_info.transaction_amount.value * 1;
        ...
        // currrent currency
        let cur_currency =
          el.currency || el.transaction_info.transaction_amount.currency_code;
  ...
  static calculateTransactionNet(req, res, next) {
    ...
    let { net_NGN, net_USD, net_GBP } = TransactionHelper.setCurrencyNet();
